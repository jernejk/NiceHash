...
$pluginProjectFile = "$basePath\NiceHash.ElgatoStreamDeck.csproj"
...
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"NiceHash.ElgatoStreamDeck`" project before calling this script.";
  exit 1;
}
